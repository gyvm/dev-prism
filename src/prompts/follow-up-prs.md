...
- `section.id`: 分析ID（`follow-up-prs`）
- `week.start` / `week.end`: 対象週のISO 8601タイムスタンプ
- `prs`: 対象期間に活動のあったPR配列。`bodyText` / `comments` / `reviews` / `reviewThreads`（`isResolved`含む）等を含む

各PRについて、レビューコメント、PRコメント、レビュースレッドの解決状態、TODO表現、延期された作業、リファクタ計画、後続チケットへの言及、そして実装方針をめぐる議論を確認してください。
...
PRごとに「次に何かのアクションが必要か」を見て4つに分類する。
...
- **💬 議論**: 実装方針・設計トレードオフ・レビューでの反対意見・要求変更・方向転換など、**意味のある議論**がコメントやレビューで確認できるもの。通常のapproveや小さなnitは含めない。論点と、確認できる結論（または「未確認」）を書く。週次MTGで振り返る価値のある論点として扱う。
...
### 💬 議論（実装方針・設計トレードオフ）
- **[owner/repo#J](https://github.com/owner/repo/pull/J)** — <議論の論点>。結果: <確認できる結論、または「未確認」>。根拠: <PRデータ上の短い根拠>
```
...
- 4カテゴリすべて該当なしの場合は、対象条件段落の直後に `対象なし。` のみを置き、`###` 見出しを出さない。
- 「議論」のバーは高めに保つ。1〜2件しか該当しない週は珍しくない。無理に水増ししない。
...
- カテゴリは `### 📌 TODO`・`### 🔍 NOTE`・`### ❓ 要確認`・`### 💬 議論` の4種のみ。bullet内で `**TODO**` のように再度ラベル付けしない。
