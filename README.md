...
| Action | 種類 | 役割 |
|---|---|---|
| `your-org/dev-prism@v0` | Docker コンテナ | PR 収集 → `data/dwh` (parquet) を増分更新 |
| `your-org/dev-prism/site@v0` | composite | `data/dwh` から Explore + Reports サイトを `dist/` にビルド |
...
> **GHCR プリビルドイメージについて**: リリース (`.github/workflows/release.yml` で tag を打つと
> `publish-image.yml` が GHCR に publish) 後は、`action.yml` の `image: 'Dockerfile'` を
> `image: 'docker://ghcr.io/your-org/dev-prism:0'` に切り替えると、consumer 側の毎 run の
> イメージビルドが消えて高速化します (現状は毎 run ビルド)。`:0` は v0 系の最新を指す浮動タグです。
...
| `ghcr.io/your-org/dev-prism:0` | 浮動イメージタグ (v0 系最新) | `action.yml` を `docker://` に切替時 / Pattern C |
| `ghcr.io/your-org/dev-prism:0.1.0` | 固定イメージタグ | docker pull でのバージョン固定 |

> 安定したら `major` bump で **v1** を切り、`@v0` 利用者を `@v1` に案内します。それまでは
> `action.yml` は `image: 'Dockerfile'` のまま (毎 run ビルド) でも動きます。GHCR を使った高速化に
> 切り替える場合は `image: 'docker://ghcr.io/your-org/dev-prism:0'` に変更してください。
