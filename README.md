...
- **Reports ギャラリー** (`/`): `report:dwh` が出力する `dist/reports/index.json` から **ビルド時に SSG**。各カードは凍結レポート (`/reports/<id>.html`) へリンク。
- **Explore** (`/explore`): `client:only` の React 島。ブラウザ内 **DuckDB-WASM** が `dist/data/*.parquet` を直接クエリし、レポートと**同一のレンダラ・SQL**で DORA / レビュー相関 / PR タイムラインをライブ集計。期間プリセット (今週/過去1ヶ月/3ヶ月/1年) + カレンダー、repo/user の multiselect で絞り込み。
- **サイドバー**: アプリ面 (一覧/Explore) は Astro が SSR。凍結レポートには閲覧時に `nav.js` がオーバーレイ描画する (本文は自己完結のまま・常に最新ナビ)。
...
| `npm run web:build` | `nav.js` ビルド + `astro build` (本番 base `/pr-weekly-report`) |
...
npm run explore:data -- --dwh-dir /tmp/dwh                                   # parquet → src/web/public/data → dist/data
...
> **注意**: 描画の更新 (デザイン変更) は `report:dwh` を再実行すれば凍結レポートを現行レンダラで再生成できます (データ凍結 / 描画オンデマンド再生成)。サイドバーの更新は `nav.js` の再デプロイのみで反映され、レポート再生成は不要です。

> **未対応 (follow-up)**: `weekly.yml` (本番 CI) はまだ旧 `report` (orchestrate) パイプライン。DWH/Astro フローへの切替・本番での parquet 配信・レポート履歴の永続化は別タスクです (`docs/astro-migration-plan.md` の §0 参照)。

## エンジンを Action として使う (別リポジトリから)

このリポジトリは **2 つの GitHub Action** として配布できます。利用側 (consumer) リポジトリは設定とデータだけを持ち、エンジン本体は versioned 参照するだけです。
...
**すぐ使えるテンプレートを [`template/`](template/) に用意しています** — config.toml + ワークフロー (collect → commit data/dwh → site build → Pages deploy) + README。コピーして `__OWNER__/__REPO__` を置換し、`GH_INSIGHTS_TOKEN` を設定して Pages を有効化するだけです。
...
- **レート制限時**: Action は取得済み分を書き込んで success (exit 0) で終了し、stderr に reset 時刻を出します。次回実行 (reset 後の手動トリガまたはスケジュール) で DWH カーソルが自動的に続きから再開します。

### サイトビルド Action の要点
- **入力**: `dwh-dir` (既定 `data/dwh`) / `base` (Astro base。Pages project page は `/<repo>/`、独自ドメイン・Cloudflare は `/`) / `output-dir` (既定 `dist`) / `reports-config` (任意・指定すると frozen reports で gallery を生成) / `site` (任意・canonical URL)。
- composite なので `${{ github.action_path }}` (= エンジンの checkout) で `npm ci` + astro ビルドを実行し、`dist/` を consumer ワークスペースへ出力します。Explore はランタイムで `/data/*.parquet` を読むため、データ更新とサイトビルドはカデンスを分けられます。
- v1 のコンテナイメージは `image: 'Dockerfile'` (毎 run ビルド)。公開時は `docker://ghcr.io/...` のプリビルドへ切り替えるのが望ましい (follow-up)。
