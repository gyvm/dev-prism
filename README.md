...
- **認証**: `GITHUB_TOKEN` を **action を呼ぶ step の `env:` で渡す**。composite の run step は
  呼び出し step の env を継承するので、収集処理がそこからトークンを読む。GHES では
  `GITHUB_API_URL` / `GITHUB_GRAPHQL_URL` がランナーから自動で渡る
- **進捗ログ**: 各フェーズ (`npm ci` / 収集 / Explore データ配置 / astro build / dist 書き出し) を
  `::group::` でグルーピングし、節目に `::notice::` を出すので Actions ログで進行が追える
- **レート制限時**: 収集は取得済み分を書き込んで success (exit 0) で終了し、stderr に reset 時刻を出す。
