...
このリポジトリは **Docker コンテナ Action** としても配布できます。利用側 (consumer) リポジトリは設定とデータだけを持ち、エンジン本体は versioned 参照するだけです。Action は PR を収集して consumer の `data/dwh/` (parquet) を増分更新し、後段で consumer がそれをコミットします。

```yaml
# consumer-repo/.github/workflows/refresh.yml
name: Refresh PR DWH
on:
  schedule: [{ cron: "0 0 * * 1" }]
  workflow_dispatch:
    inputs:
      from: { description: "初回 backfill 開始日 (YYYY-MM-DD)", required: false }
permissions:
  contents: write
jobs:
  build:
    ...
    steps:
      ...
      - uses: your-org/pr-weekly-report@v1
        with:
          config: config.toml
          from: ${{ github.event.inputs.from }}
        env:
          # Docker コンテナ Action には GITHUB_TOKEN が自動注入されないため、
          # 必ず env で渡す (read-only PAT か GITHUB_APP_* を使用)。
          GITHUB_TOKEN: ${{ secrets.GH_INSIGHTS_TOKEN }}
      - run: |
          git config user.name "github-actions[bot]"
          git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
          git add data/dwh
          git commit -m "chore: refresh DWH" || echo "no changes"
          git push
```
