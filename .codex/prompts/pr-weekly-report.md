...
- 対象リポジトリは `--repositories "owner/name"` で渡す。省略時はカレントの git remote から推定される

## 手順

1. **fetch**: `npx pr-weekly-report fetch [--repositories "owner/*"] [--week <YYYY-MM-DD>]` を実行。stdout の JSONL パスを記録 (`<JSONL>`)
...
- fetch 失敗: `--repositories` 指定 (または git remote) と PAT を確認
