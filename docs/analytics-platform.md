...
- **一覧は `index.json` だけ読む**(全 HTML をパースしない)。`buildIndexHtml` 相当を、ファイル
  走査ではなく `index.json` ベースに置き換え、scope/タイトル/ハイライトを出せるようにする。
- (任意)将来「旧レポートを新デザインで作り直す」可能性に備えるなら、生成時の集計結果を
  `<script type="application/json">` で同梱しておけば、**当時の数値のまま再スキン**できる。
  既存出力にこの素データ同梱は無いので、**必要になったら足す**(現状同等を優先するなら省略可)。
