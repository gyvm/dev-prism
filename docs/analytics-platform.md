...
- **稀な取りこぼし**:`updatedAt` を上げない種の変更、PR/repo ごと削除は反映されない →
  必要なら**定期フル再同期**で補正(注記)。
- 実装変更は4点:**① GraphQL に `id` / `updatedAt` / actor の `__typename` を追加、② child
  connection をページング完走、③ normalize に `source_node_id` と typed fact を伝播、④
  検索を `updated:>=` 化**(現状は `src/collector/graphql.ts` の `created:>=`)。
...
3. **増分収集 + upsert**:GraphQL に `id` / `updatedAt` / actor `__typename` を追加し検索を
   `created:>=` → `updated:>=` へ変更。reviews / reviewRequests / timelineItems / comments /
   reviewThreads / commits / files の child connection をページング完走し、`max(updated_at)−overlap`
   をカーソルに PR 単位で冪等 upsert(取り込み後にスナップショット破棄)。
