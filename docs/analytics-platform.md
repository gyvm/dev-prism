...
- **稀な取りこぼし**:`updatedAt` を上げない種の変更、PR/repo ごと削除は反映されない →
  必要なら**定期フル再同期**で補正(注記)。
- 実装状況:**① GraphQL の `id` / `updatedAt` / actor の `__typename` 取得**と**④ 検索の
  `updated:>=` 化**は **`src/collector/graphql.ts` で実装済み**(`buildSearchQuery` は既に
  `updated:>=`、ActorFields は `__typename` / `id` を含む)。残る実装は3点:**② child connection
  (reviews / comments / reviewThreads / commits / files)をページング完走**(現状は `first:100`/`50`
  の単発取得で `pageInfo` ループが無く、件数超過 PR は取りこぼす)、**③ normalize に `source_node_id`
  と typed fact を伝播**(DWH build 層が未実装)、**カーソル基準を static な `cutoffDate` から
  `max(pull_requests.updated_at) − overlap` の DWH 由来ウォーターマークへ移行**(DWH 実体が
  できてから)。
...
3. **増分収集 + upsert**:GraphQL の `id` / `updatedAt` / actor `__typename` 取得と検索の
   `updated:>=` 化は **実装済み**(`src/collector/graphql.ts`)。残作業は、reviews / reviewRequests /
   timelineItems / comments / reviewThreads / commits / files の child connection を `pageInfo` で
   ページング完走(現状は単発取得)、カーソルを static `cutoffDate` から `max(updated_at)−overlap` へ
   移行、PR 単位で冪等 upsert(取り込み後にスナップショット破棄)。
