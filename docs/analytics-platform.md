...
- スター + ロング型 `activities` / 型付き `pull_requests` / ブリッジ / dims / `bodies`、typed×JSON ハイブリッド(ER 図あり)
...
- 増分:PR `updatedAt` ウォーターマーク、PR 単位 full-replace 冪等 upsert / 正規化は build 時
...
- デプロイ:静的でどこでも(private は要保護)

**未確定(同じ高さで残る)**
1. 配布チャネルの主軸(Action 主 + Docker 従 を推奨だが保留)
2. Explore 既定ダッシュボードの指標セット(既存3分析 + 件数推移 を既定に、で可か)
3. private データの既定公開スタンス(public Pages / private は Cloudflare Access)
